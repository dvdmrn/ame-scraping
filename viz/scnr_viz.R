...
mean_data <- group_by(scenario_raw, game) %>%
  summarise(score = mean(score, na.rm = TRUE)) 
scenario_raw$game <- as.numeric(scenario_raw$game)
head(mean_data)

dat2 <- ddply(scenario_raw, .(game), summarise, 
              M = mean(score), SE = sd(score) / sqrt((length(score))), 
              SD = sd(score))
names(dat2)[2] <- "score"
dat2

# p <- ggplot(scenario_raw, aes(x=game,y=score)) +
